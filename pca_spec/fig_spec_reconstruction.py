...
evals_cs = evals.cumsum()
evals_cs /= evals_cs[-1]
evecs = data['evecs'] # [n_comp,n_feature], should be the transpse of eigen-vector matrix
# i.e, the nature output of pca.components_
...
spec = spectra[1,:]
coeff = np.dot(evecs, spec - spec_mean)  #i.e np.dot(eigen_vec,single_sample-sample_mean)
#textbook, p300, (7.17)

#[nfeature,nfeature]x[nfeature,] ->[nfeature,]
...
for i, n in enumerate([0, 4, 8, 20]):
    ...
    ax.plot(wavelengths, spec_mean + np.dot(coeff[:n], evecs[:n,:]), '-k') #[1,n]x[n,nfeature]=[1,nfeature]
    #textbook, p300, (7.18)
